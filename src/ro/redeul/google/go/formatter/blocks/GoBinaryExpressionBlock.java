...
class GoBinaryExpressionBlock extends GoSyntheticBlock<GoBinaryExpression>{

    int depth;
    int cutoff;

    GoBinaryExpressionBlock(@NotNull GoBinaryExpression node, CommonCodeStyleSettings settings, Indent indent) {
        super(node, settings, indent, null, GoBlockUtil.Alignments.EMPTY_MAP);

//        if (node.precedence() > 3)
